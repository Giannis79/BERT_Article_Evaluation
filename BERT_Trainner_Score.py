...
class InfluenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        text = row['text']
        label = row['score']  # Use 'score' directly 
...
# Load Data
df = pd.read_csv('Trainer_Data.csv', encoding='latin-1')
...
def train_model(model, train_loader, epochs, learning_rate):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            labels = batch['label'].unsqueeze(1).float()  # Unsqueeze and convert to float
...
# Save Model
model.save_pretrained('sentiment_model_score')
tokenizer.save_pretrained('sentiment_model_score')